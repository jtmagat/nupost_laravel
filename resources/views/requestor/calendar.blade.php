...
.cal-grid tbody td {
    ...
    border:1px solid rgba(255,255,255,0.05); transition:background .12s; cursor:default;
}
.cal-grid tbody td.has-events { cursor:pointer; }
...
.cal-grid tbody td.cal-day--other { background:rgba(0,0,0,0.12); }
...
.cal-event--others  { background:rgba(255,255,255,0.12); color:rgba(255,255,255,0.55); font-style:italic; }
.cal-event--mine    { border-left:2.5px solid #f97316; }
...
.upcoming-mine-tag   { display:inline-block; padding:1px 8px; background:#dbeafe; color:#1d4ed8; border-radius:10px; font-size:10px; font-weight:700; margin-left:6px; }
...
.dp__body { padding:12px 14px; max-height:480px; overflow-y:auto; }
...
.req-tag--mine    { background:#dbeafe; color:#1d4ed8; }
...
@php
    ...
    // Private mode: only current user's requests with preferred_date
    // Public mode:  all requests with preferred_date
    if ($is_public) {
    ...
    if ($is_public) {
        $upcoming = \App\Models\PostRequest::whereNotNull('preferred_date')
            ...
            ->orderByRaw("FIELD(priority,'Urgent','High','Medium','Low')")
            ->get()
            ->map(function($r) use ($user_name){ $r->is_mine = ($r->requester === $user_name); return $r; });
    } else {
        $upcoming = \App\Models\PostRequest::whereNotNull('preferred_date')
            ->where('requester', $user_name)
            ->whereBetween('preferred_date',[now()->format('Y-m-d'),now()->addDays(7)->format('Y-m-d')])
...
<<<<<<< HEAD
        <span><strong>Public view is ON</strong> — Showing all users' preferred posting dates. Your requests have an orange left border. Dates with many requests are highlighted red — pick a less busy date.</span>
    </div>
    @endif
            ...
            <div class="cal-card">
                        ...
                        for ($i = 0; $i < $total_cells; $i++) {
                            ...
                            if ($is_current && $cnt > 0) {
                                ...
                                foreach ($day_evs as $ev) {
                                    ...
                                    $is_mine = ($ev->requester === $user_name);
                                    $mine_c  = ($is_public && $is_mine) ? ' cal-event--mine' : '';
                                    $cls     = $is_public && !$is_mine ? 'cal-event cal-event--others' : "cal-event " . $prio_ev_cls($p) . $mine_c;
                ...
                <div class="cal-legend">
                    @if($is_public)
                        ...
                        <div class="legend-item"><span class="legend-dot" style="background:rgba(148,163,184,0.45);"></span>Others</div>
                        <div class="legend-item"><span class="legend-dot" style="border-radius:50%;background:#f59e0b;"></span>Today</div>
                        <div class="legend-item"><span class="legend-dot" style="border-radius:50%;background:rgba(239,68,68,0.6);"></span>Busy</div>
                    @else
                        <div class="legend-item"><span class="legend-dot" style="background:rgba(239,68,68,0.85);"></span>Urgent</div>
                        <div class="legend-item"><span class="legend-dot" style="background:rgba(245,158,11,0.85);"></span>High</div>
                        <div class="legend-item"><span class="legend-dot" style="background:rgba(59,110,245,0.8);"></span>Medium</div>
...
function showPanel(el) {
    ...
    // Already sorted by PHP (priority + created_at), but re-sort client side for safety
    evs.sort((a,b)=>{
    ...
    document.getElementById('panel-count').textContent = evs.length + ' request' + (evs.length!==1?'s':'') + ' — by priority · earliest first';
    ...
    evs.forEach((ev,i)=>{
        const mineTag = IS_PUBLIC
            ...
            : '';
        html += `
        <div class="req-card ${cardCls(ev.priority)}">
