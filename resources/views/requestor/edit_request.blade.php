...
.locked-banner { display: flex; align-items: center; gap: 10px; background: #fef3c7; border: 1px solid #fde68a; border-radius: 8px; padding: 10px 14px; margin-bottom: 16px; font-size: 12.5px; color: #92400e; }
...
/* Current media */
.current-media { display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 10px; }
.current-media-item { position: relative; width: 80px; height: 80px; }
.current-media-item img { width: 80px; height: 80px; border-radius: 8px; object-fit: cover; border: 2px solid #a7f3d0; }
.current-media-label { position: absolute; bottom: 0; left: 0; right: 0; background: rgba(0,0,0,0.5); color: white; font-size: 9px; text-align: center; padding: 2px; border-radius: 0 0 6px 6px; }
/* AI Section */
...
<main class="main">
    ...
    <div class="page-header">
        <h1>Edit Request</h1>
        <p>Update your pending request — only available while status is "Pending Review"</p>
    </div>

    <div class="locked-banner">
        <svg width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
        This request is still <strong>Pending Review</strong> — you can still edit it. Once the admin reviews it, editing will be locked.
    </div>
    ...
    <form method="POST" action="{{ route('requestor.requests.update', $req->id) }}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="form-section">
            ...
            <div class="field">
                ...
                <input type="text" name="title" value="{{ old('title', $req->title) }}"
                       placeholder="e.g., College Week 2025 Opening Ceremony" required>
            </div>
            <div class="field">
                ...
                <div class="field">
                    ...
                    <select name="category" required>
                        ...
                        @foreach(['Events','Announcements','Academic','Sports','Community','Others'] as $cat)
                            <option value="{{ $cat }}" {{ old('category', $req->category) === $cat ? 'selected' : '' }}>{{ $cat }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="field">
                    ...
                    <select name="priority" required>
                        ...
                        @foreach(['Low','Medium','High','Urgent'] as $pri)
                            <option value="{{ $pri }}" {{ old('priority', $req->priority) === $pri ? 'selected' : '' }}>{{ $pri }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="form-section">
            ...
            @if($req->media_files && count($req->media_files) > 0)
                <p style="font-size:12px;color:var(--color-text-muted);margin-bottom:8px;">Current media (will be kept unless you upload new ones):</p>
                <div class="current-media">
                    @foreach($req->media_files as $mf)
                        <div class="current-media-item">
                            ...
                            <div class="current-media-label">Current</div>
                        </div>
                    @endforeach
                </div>
            @endif
            ...
            <p class="upload-hint" style="margin-top:8px;">Upload new files to replace existing ones (Max 4, 10MB each)</p>
        </div>
        ...
        <div class="ai-section">
            ...
            <textarea class="ai-textarea" name="caption" id="caption-field"
                      placeholder="Write or generate an AI-powered caption...">{{ old('caption', $req->caption) }}</textarea>
        </div>
...
async function generateCaption() {
    ...
    try {
        ...
        if (data.caption && captionBox) captionBox.value = data.caption;
